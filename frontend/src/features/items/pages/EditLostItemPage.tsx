...
import { editLostItemSchema, type EditLostItemFormValues } from "../schemas/editLostItemSchema";
import {
  getLostItem,
  updateLostItem,
  replaceLostItemPhoto,
  deleteLostItemPhoto,
...
export function EditLostItemPage() {
  ...
  } = useForm<EditLostItemFormValues>({
  ...
  useEffect(() => {
    ...
    getLostItem(id, controller.signal)
      .then((data) => {
        setItem(data);
        reset({
          title: data.title,
          ...
          hiddenInformation: "", // never returned by the API — user re-enters to change it
        });
      })
      .catch((err) => {
  ...
  const onSubmit = async (values: EditLostItemFormValues) => {
    ...
    setSaved(false);
    try {
      const updated = await updateLostItem(id, values);
      setItem(updated);
      setSaved(true);
    } catch (err) {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-6">
          <div className="rounded-2xl border border-gray-100 bg-white p-8 shadow-sm">
            ...
            <div className="mt-6 grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              <WizardField id="lastKnownLocation" label="Last Known Location" required error={errors.lastKnownLocation?.message}>
                <input
                  id="lastKnownLocation"
                  type="text"
                  {...register("lastKnownLocation")}
                  className={wizardInputClass(!!errors.lastKnownLocation)}
                />
              </WizardField>
            </div>
          </div>
