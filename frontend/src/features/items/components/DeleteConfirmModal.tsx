...
export function DeleteConfirmModal({ item, isSubmitting, onConfirm, onCancel }: DeleteConfirmModalProps) {
  return (
    <div
    ...
    >
      <div className="relative w-full max-w-md rounded-3xl bg-white p-8 shadow-xl">
        ...
        <div className="flex flex-col items-center text-center">
          <span className="mb-5 flex h-16 w-16 items-center justify-center rounded-full bg-red-50">
            <Trash2 className="h-7 w-7 text-red-600" strokeWidth={2.5} />
          </span>
          ...
          <p className="mt-3 text-[15px] text-gray-500">
            This will remove it from browse and search results. This action can't be undone from here.
          </p>
        </div>
        ...
        <div className="mt-6 flex gap-3">
          ...
          <button
            ...
            onClick={onConfirm}
            disabled={isSubmitting}
            className="flex-1 rounded-xl bg-red-600 py-3 text-sm font-semibold text-white transition-opacity hover:opacity-95 disabled:opacity-60"
          >
